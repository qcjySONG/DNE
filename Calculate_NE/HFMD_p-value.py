...
for gap_year in range(8,9):
    ...
    It_5 = []  # 记录后5个It值
    num=0
    for f in range(15, 21):  # 由于程序编写的问题 1-21为2000-2024的
        '''
        #年：{
            地区：{It分数，每周}
            预警点
        ...
        book = xlrd.open_workbook(
            'data/东京都手足口病定点报告数.xls')  # 应该是点的内容  'data/东京都手足口病定点报告数.xls' 'data/东京都流感定点报告数.xls' 'data/东京都肠胃炎定点报告数.xls'
        ...
        for i in range(1, len(It_p_list)):
            ...
            if It_p_list[i] == 0:
                It_p_list[i] = 1
            if It_p_list[i] < 0.005 and It[i] > ((0.5 * 4) / 23):  # HMDL  abs(It[i]-It[i-1])>((0.1*4)/23)         FLU   It[i]>((0.5*4)/23)
                #print(It_p_list[i])
                It_p_list[i] = (0.05 + It_p_list[i]) / 2
            if It_p_list[i] < 0.005 and It[i] < ((0.5 * 4) / 23):
        ...
        if mild.__len__() > 0:
            for i in range(mild.__len__()):
                ...
                ax.add_artist(AnnotationBbox(icon, (mild[i] + 1, It_p_list[mild[i]]), frameon=False))  # 添加图标
        ...
        myfig.tight_layout()
        myfig.subplots_adjust(left=0.12, right=0.83, top=0.94, bottom=0.12)
        # myfig.subplots_adjust(left=0.12, right=0.85, top=0.98, bottom=0.12)#HFMD
        myfig.savefig(f'son_img_HMDL/testTokyo_{f}.png', dpi=150)
        plt.close(myfig)
